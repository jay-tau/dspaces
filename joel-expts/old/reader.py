    ...
    for pattern in patterns:
        for i in range(iterations):
            if pattern == "contiguous":
                ...
                ub = tuple(
                    dim[j] - 1 if j > 0 else rank * local_dim[0] + local_dim[0] - 1
                    for j in range(ndim)
                )
                ...
                ub = tuple(
                    dim[j] - 1
                    if j > 0
                    else rank * local_dim[0] * 2 + (local_dim[0] // 2) - 1
                    for j in range(ndim)
                )
                ...
                ub = tuple(
                    dim[j] - 1 if j > 0 else rand_pos + local_dim[0] - 1
                    for j in range(ndim)
                )

            start_time = time.time()
            data = client.Get(
                f"{ndim}D_{pattern}_data", version=i, lb=lb, ub=ub, timeout=-1
            )
            end_time = time.time()
...
if rank == 0:
    for dim in dims:
        ...
        for pattern in patterns:
            ...
            print(
                f"Reader {ndim}D {pattern} pattern - Avg time: {avg_time:.4f} s, "
