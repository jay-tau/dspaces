...
class TestWriteBenchmark:
    ...
    def test_contiguous_write(self, benchmark, ds_client, dim):
        ...
        def run_write():
            ...
            ds_client.Put(
                data, f"{len(dim)}D_contiguous", version=version[0], offset=offset
            )
            version[0] += 1
    ...
    def test_strided_write(self, benchmark, ds_client, dim):
        ...
        def run_write():
            ...
            offset = tuple(0 for _ in range(len(dim)))
            ds_client.Put(
                data, f"{len(dim)}D_strided", version=version[0], offset=offset
            )
            version[0] += 1
...
class TestReadBenchmark:
    ...
    def test_contiguous_read(self, benchmark, ds_client, dim):
        ...
        def run_read():
            result = ds_client.Get(
                f"{len(dim)}D_contiguous", version=version[0], lb=lb, ub=ub, timeout=-1
            )
    ...
    def test_strided_read(self, benchmark, ds_client, dim):
        ...
        def run_read():
            result = ds_client.Get(
                f"{len(dim)}D_strided", version=version[0], lb=lb, ub=ub, timeout=-1
            )
            version[0] = (version[0] + 1) % BenchmarkConfig.MAX_VERSIONS
            return result
    ...
    def test_random_read(self, benchmark, ds_client, dim):
        ...
        def run_read():
            ...
            result = ds_client.Get(
                f"{len(dim)}D_random", version=version[0], lb=lb, ub=ub, timeout=-1
            )
            version[0] = (version[0] + 1) % BenchmarkConfig.MAX_VERSIONS
            return result
